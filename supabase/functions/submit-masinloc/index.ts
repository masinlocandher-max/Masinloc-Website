...
const supabase=createClient(Deno.env.get("SUPABASE_URL")!,SERVICE_ROLE_KEY,{auth:{persistSession:false}});
/* Hashing an IP with a bare digest is reversible: the whole IPv4 space is
   four billion values, which is minutes of work. Keying the hash means a
   leaked security_events table cannot be walked back to addresses. */
...
/* An exact list. This used to trust any masinloc-website-*.vercel.app or
   masinloc-connect-*.vercel.app host, which is a production submission
   endpoint trusting a pattern rather than a place. */
...
const configs={
  ...
  /* Non-emergency messages to the PNP and MDRRMO desks. No attachments:
     a desk reading these is not equipped to open arbitrary files, and
     every byte accepted from an unauthenticated form is surface. */
  assistance:{table:"assistance_reports",bucket:null,maxFiles:0,maxFileBytes:0,mime:[]},
} as const;
...
async function hmac256(value:string){const key=await crypto.subtle.importKey("raw",new TextEncoder().encode(SECURITY_HASH_KEY),{name:"HMAC",hash:"SHA-256"},false,["sign"]);const signature=await crypto.subtle.sign("HMAC",key,new TextEncoder().encode(value));return[...new Uint8Array(signature)].map(b=>b.toString(16).padStart(2,"0")).join("")}
...
async function consumeRate(fingerprint:string,limit:number,windowSeconds=900){const{data,error}=await supabase.rpc("check_submission_rate_limit",{p_fingerprint:fingerprint,p_limit:limit,p_window_seconds:windowSeconds});if(error)throw new Error("SERVER");return data===true}
async function checkRate(req:Request,category:string,perIpLimit=8,globalLimit=2000){const perIp=await hmac256(`${clientIp(req)}|${category}`);if(!(await consumeRate(perIp,perIpLimit)))return false;const global=await hmac256(`global|${category}`);return consumeRate(global,globalLimit)}
async function verifyTurnstile(req:Request,token:string){if(!TURNSTILE_REQUIRED)return true;if(!TURNSTILE_SECRET_KEY||!token)return false;const body=new URLSearchParams();body.set("secret",TURNSTILE_SECRET_KEY);body.set("response",token);const ip=clientIp(req);if(ip&&ip!=="unknown")body.set("remoteip",ip);const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),5000);try{const r=await fetch("https://challenges.cloudflare.com/turnstile/v0/siteverify",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body,signal:controller.signal});const result=await r.json();if(!result?.success)return false;if(result.action&&result.action!=="masinloc_submit")return false;if(result.hostname&&!ALLOWED_ORIGINS.has(`https://${String(result.hostname)}`))return false;return true}catch{return false}finally{clearTimeout(timer)}}
...
function rowFor(category:Category,p:Record<string,any>,files:string[]){
  ...
  if(category==="assistance"){
    required(p,["deskCode","reportKind","subject","body"]);
    const desks=["pnp","mdrrmo"];
    const kinds=["blotter_followup","safety_concern","hazard_report","assistance_request","information"];
    const deskCode=String(p.deskCode);
    const reportKind=String(p.reportKind);
    /* Validated against a list rather than passed through, so the database
       constraint is a second line of defence and not the only one. */
    if(!desks.includes(deskCode)||!kinds.includes(reportKind))throw new Error("VALIDATION");
    /* A contact is optional — somebody reporting a recurring hazard may have
       good reason not to leave a name — but a stub is worse than nothing,
       because a desk will try to use it. */
    const contact=p.reporterContact?text(p.reporterContact,200):null;
    if(contact!==null&&contact.length<5)throw new Error("VALIDATION");
    return{
      desk_code:deskCode,
      report_kind:reportKind,
      subject:text(p.subject,200),
      body:text(p.body,6000),
      barangay:p.barangay?text(p.barangay,120):null,
      reporter_name:p.reporterName?text(p.reporterName,160):null,
