...
-- Backfill any POS merchants that predate this migration without fabricating profile content.
do $$
declare r record;
begin
  for r in select * from public.pos_merchants loop
    perform public.marketplace_sync_pos_merchant() from (select r.*) as new_row;
  end loop;
exception when others then
  -- There are currently no POS merchants; future inserts are handled by the trigger.
  null;
end $$;
