...
--
-- Intake is the edge function (service role); the browser never reads this
-- table. Contributor names reach the public page only through the function's
-- read-only contributors endpoint, and only once an admin has approved the
-- row, so contact details stay unreachable from the client by construction.
