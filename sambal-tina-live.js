(() => {
  ...
  function communityRows() {
    const layer = window.SAMBAL_TINA_COMMUNITY;
    if (!layer || !Array.isArray(layer.entries)) return [];
    return layer.entries.map((item) => ({
  ...
  function bytesFromBase64(value) {
    ...
    if (!encoded || encoded.length % 4 !== 0) {
      throw new Error('The dictionary source payload has an invalid base64 length.');
    }
  ...
  async function inflateSource() {
    if (!window.SAMBAL_TINA_DATA) throw new Error('The dictionary source payload is unavailable.');
    if (!('DecompressionStream' in window)) {
      throw new Error('This dictionary needs a current browser with built-in gzip support.');
    }

    const bytes = bytesFromBase64(window.SAMBAL_TINA_DATA);
    const stream = new Blob([bytes]).stream().pipeThrough(new DecompressionStream('gzip'));
    const json = await new Response(stream).text();
    const rows = JSON.parse(json);

    if (!Array.isArray(rows) || rows.length !== SOURCE_ENTRY_COUNT) {
      throw new Error('The dictionary source did not pass its 5,222-entry integrity check.');
    }

    const reviewCount = rows.reduce((count, row) => count + (Number(row?.[4]) === 1 ? 1 : 0), 0);
  ...
  function mergeLayers() {
    ...
    const source = state.sourceEntries.map((entry) => {
    ...
    state.entries = [...source, ...supplemental];
  }
  ...
  function renderCommunityWords() {
    ...
    visible.forEach((entry) => {
      ...
      button.append(make('strong', '', entry.word), make('span', '', entry.filipino || entry.english));
      button.addEventListener('click', () => searchFor(entry.word));
      root.append(button);
    });
  ...
  function applyFilters() {
    ...
    state.filtered = state.entries.filter((entry) => {
      if (state.letter !== 'ALL' && initialLetter(entry.word) !== state.letter) return false;
      if (!q) return true;
      return [
        entry.word,
        entry.english,
    ...
    state.filtered.sort((a, b) => {
      if (q && a.communityConfirmed !== b.communityConfirmed) return a.communityConfirmed ? -1 : 1;
  ...
  function wordCard(entry) {
    ...
    else if (entry.communityConfirmed) meta.append(make('span', 'community-badge', 'Source + living usage'));
  ...
  function renderCards() {
    ...
    if (state.query || state.letter !== 'ALL') $('resultCount').textContent = `${total.toLocaleString()} ${total === 1 ? 'result' : 'results'}`;
    else if (state.sourceReady) $('resultCount').textContent = `${SOURCE_ENTRY_COUNT.toLocaleString()} source entries · ${livingCount} living confirmations`;
    else $('resultCount').textContent = `${livingCount} living confirmations`;

    grid.replaceChildren();
    if (!total) grid.append(make('div', 'dictionary-empty', 'No matching entry. Try another Sambal Tina spelling, English meaning, or Filipino equivalent.'));
  ...
  async function init() {
    ...
    try {
      state.sourceEntries = await inflateSource();
