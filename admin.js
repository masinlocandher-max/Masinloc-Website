...
async function openDetail(id){
 ...
 $('#detailContent').innerHTML=`<p class="detail-kicker">${esc(cfg.label)}</p><h2 class="detail-title">${esc(cfg.title(r)||'Record')}</h2><p class="detail-ref">${refLine}</p>${activeType==='security'&&r.ip_address?'<p class="muted">Raw IP is temporarily retained for this high-severity event. It will be cleared automatically while the hashed fingerprint remains for pattern detection.</p>':''}${activeType==='contact'?'<p class="muted">Reply to the sender directly at the address above, then set the status. Nothing here is ever shown publicly.</p>':''}${activeType==='dictionary'?'<p class="muted">Check whether this word or correction already exists in the collection during review. Intake accepts genuine submissions without requiring the contributor to check first.</p>':''}${activeType==='entries'&&r.status==='draft'&&r.layer==='correction'?'<p class="muted"><strong>This reading is not on the public site.</strong> Its glyphs could not be resolved from the printed page, so it is withheld rather than published \u2014 the internal note says which archive page to open and which half of the entry is damaged. Correct the word here, then set the status to Published to put it back into the dictionary. Leave it as a draft and it stays hidden.</p>':''}<div class="detail-grid">${fields}</div>${attachments?`<div class="detail-field full" style="margin-top:12px"><span>Private attachments</span><div class="attachment-list">${attachments}</div></div>`:''}${editor}`;
